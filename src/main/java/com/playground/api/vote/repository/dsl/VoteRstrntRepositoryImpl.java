...
public class VoteRstrntRepositoryImpl implements VoteRstrntRepositoryCustom {
  ...
  public List<VoteRstrntResponse> getVoteRstrntList() {
    ...
    return queryFactory
        .select(Projections.fields(VoteRstrntResponse.class
            ,voteEntity.voteSn.as("voteSsno")
            ,voteEntity.voteKndCode.as("voteKindCode")
            ,voteEntity.voteSj.as("voteSubject")
            ,voteEntity.annymtyVoteAt.as("anonymityVoteYn")
            ,voteEntity.voteBeginDt.as("voteBeginDate")
            ,voteEntity.voteEndDt.as("voteEndDate")
            ,voteEntity.voteDeleteAt.as("voteDeleteYn")
            ,qestnEntity.qestnSn.as("qestnSsno")
            ,qestnEntity.qestnCn.as("qestnName")
            ,qestnEntity.compnoChoiseAt.as("compnoChoiseYn")
            ))
        .from(voteEntity)
        .leftJoin(qestnEntity)
        .on(voteEntity.voteSn.eq(qestnEntity.voteSn))
        .leftJoin(voteIemEntity)
        .on(voteEntity.voteSn.eq(voteIemEntity.voteSn).and(qestnEntity.qestnSn.eq(voteIemEntity.qestnSn)))
        .join(rstrntEntity)
        ...
        .transform(groupBy(voteEntity.voteSn, qestnEntity.qestnSn)
            .list(Projections.fields(VoteRstrntResponse.class
                ,voteEntity.voteSn.as("voteSsno")
                ,voteEntity.voteKndCode.as("voteKindCode")
                ,voteEntity.voteSj.as("voteSubject")
                ,voteEntity.annymtyVoteAt.as("anonymityVoteYn")
                ...
                ,qestnEntity.qestnCn.as("qestnName")
                ,qestnEntity.compnoChoiseAt.as("compnoChoiseYn"),
                list(Projections.fields(VoteRstrntIemResponse.class, 
                    voteIemEntity.iemSn.as("iemSsno"), 
                    voteIemEntity.iemNm.as("iemName")))
