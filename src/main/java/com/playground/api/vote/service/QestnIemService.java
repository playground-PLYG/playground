...
public class QestnIemService {
  ...
  public List<QestnResponse> addQestn(List<QestnRequest> qestnReqList) {
    ...
    for (QestnRequest qestn : qestnReqList) {
      resEntityList.add(QestnEntity.builder().voteSn(qestn.getVoteSsno()).qestnCn(qestn.getQuestionContents())
    ...
    return saveAllEntities.stream().map(entity -> QestnResponse.builder().questionSsno(entity.getQestnSn()).voteSsno(entity.getVoteSn())
  ...
  public List<QestnResponse> modifyQestn(List<QestnRequest> qestnReqList) {
    List<QestnEntity> resEntityList = new ArrayList<>();
    for (QestnRequest qestn : qestnReqList) {
      QestnEntity resQestn = qestnRepository.save(QestnEntity.builder().qestnSn(qestn.getQuestionSsno()).voteSn(qestn.getVoteSsno())
    ...
    return resEntityList.stream().map(entity -> QestnResponse.builder().questionSsno(entity.getQestnSn()).voteSsno(entity.getVoteSn())
        .questionContents(entity.getQestnCn()).compoundNumberChoiceAlternative(entity.getCompnoChoiseAt()).build()).toList();
  }
  ...
  public List<VoteIemResponse> addVoteIem(List<VoteIemRequest> voteIemReqList) {
    ...
    for (VoteIemRequest voteIem : voteIemReqList) {
      resEntityList.add(VoteIemEntity.builder().voteSn(voteIem.getVoteSsno()).qestnSn(voteIem.getQuestionSsno()).iemSn(voteIem.getItemSsno())
          .iemNm(voteIem.getItemName()).build());
    }

    List<VoteIemEntity> saveAllEntities = voteIemRepository.saveAll(resEntityList);
    return saveAllEntities.stream().map(entity -> VoteIemResponse.builder().voteSsno(entity.getVoteSn()).questionSsno(entity.getQestnSn())
  ...
  public List<VoteIemResponse> modifyVoteIem(List<VoteIemRequest> voteIemReqList) {
    List<VoteIemEntity> resEntityList = new ArrayList<>();
    for (VoteIemRequest voteIem : voteIemReqList) {
      VoteIemEntity resVoteIem = voteIemRepository.save(VoteIemEntity.builder().voteSn(voteIem.getVoteSsno()).qestnSn(voteIem.getQuestionSsno())
          .iemSn(voteIem.getItemSsno()).iemNm(voteIem.getItemName()).build());

      resEntityList.add(resVoteIem);
    }

    return resEntityList.stream().map(entity -> VoteIemResponse.builder().voteSsno(entity.getVoteSn()).questionSsno(entity.getQestnSn())
        .itemSsno(entity.getIemSn()).itemName(entity.getIemNm()).build()).toList();
  }
